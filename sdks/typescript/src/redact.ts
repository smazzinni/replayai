...
export function redactMarker(secret: string): string {
...
function applyEntropyRedaction(text: string): string {
  return text.replace(ENTROPY_CANDIDATE, (token) => {
    if (isWhitelisted(token)) return token;
    const ent = shannonEntropy(token);
    if (ent > 4.5) return redactMarker(token);
    return token;
  });
}
...
export function redactOptional(value: string | null | undefined): string | null {
  if (value === null || value === undefined) return null;
  return redactText(value);
}

/** Internal: clear the marker cache (tests only). */
export function _clearMarkerCache(): void {
  markerCache.clear();
}
