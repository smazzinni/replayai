...
let lastFlushResult: FlushResult | null = null;

export function getLastFlushResult(): FlushResult | null {
  return lastFlushResult;
}

export function _resetLastFlushResult(): void {
...
export async function flushSession(payload: FlushPayload): Promise<FlushResult> {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      const result: FlushResult = { ok: false, error: msg };
