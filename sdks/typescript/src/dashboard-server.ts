...
import { listSessions, getSession, getStats, countSessions, type LocalSession } from "./local-store.js";
...
function sendJSON(res: ServerResponse, data: unknown, status = 200, port?: number, origin?: string): void {
...
export function startServer(opts: {
  ...
  const server = createServer((req, res) => {
    ...
    if (path === "/api/stats") {
      sendJSON(res, getStats(), 200, port, origin);
      return;
    }
