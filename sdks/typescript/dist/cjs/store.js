...
exports._clearQueue = _clearQueue;
exports.newLocalSessionId = newLocalSessionId;
 ...
 */
function maybeTruncate(payload, maxSteps) {
    let truncated = false;
...
function truncateSteps(steps, head, tail) {
    ...
    for (const s of [...first, ...last]) {
        if (s.id)
            ...
            seen.add(`${s.name}|${s.t ?? s.offsetMs ?? 0}`);
    }
 ...
 */
async function flushOnce(payload) {
...
async function flushSession(payload) {
    ...
    if (truncated) {
        console.warn(`[replayai] payload for session ${payload.sessionId} exceeded limits — steps truncated (kept first 50 + last 50 + errors)`);
    }
    ...
    for (let attempt = 1; attempt <= RETRY_MAX_ATTEMPTS; attempt++) {
        try {
            ...
            if (attempt < RETRY_MAX_ATTEMPTS) {
                ...
                console.warn(`[replayai] flush attempt ${attempt}/${RETRY_MAX_ATTEMPTS} failed (${m}); retrying in ${backoff}ms`);
                await sleep(backoff);
            }
        }
    }
    ...
    if (cfg.strict) {
        throw new Error(`ReplayAI flush failed after ${RETRY_MAX_ATTEMPTS} attempts: ${m}${queued ? " (payload queued for retry)" : " (queue full — payload dropped)"}`);
    }
    console.warn(`[replayai] flush failed after ${RETRY_MAX_ATTEMPTS} attempts: ${m}${queued ? " (payload queued for retry)" : " (queue full — payload dropped)"}`);
    return {
        ok: false,
        error: m,
